...
class WritingReview extends React.Component {
  ...
  handleOnImgSubmit = (e) => {
      ...
      axios
        .post(`http://localhost:3003/api/review/product_id/${pro_id}`, formData)
        .then((res) => {
          console.log('2', res.data);
          const { filename } = res.data;
          console.log(filename);
          // this.setState({ filename: filename, filepath: `http://localhost:3003/img/${filename}` });
  ...
  render() {
    const { visible, star_hover, hover_idx, star_click, click_idx, inputValue, isTyping, isChecked, isClicked, uploadedImg, imgContent } = this.state;
    ...
    return (
      <>
      <button className="writing-review-button" onClick={this.openModal}>리뷰쓰기</button>
      {visible && <ReactModal
        visible={visible}
        maskClosable={true}
        ...
        closable={true}>
          <div className="writing-review-wrap">
            <form
              ...
              className="writing-review-form-wrap">
...
export default withRouter(WritingReview);